...
| `.github/workflows/ci.yml` | Lint, typecheck, tests, build, audit; builds the image and runs `deploy-check` inside it with synthetic values |
...
Optional: Settings → enable **Wait for CI** so a deploy starts only after the GitHub `CI` workflow passes on `main`.
